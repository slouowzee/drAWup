/**
 * Module de récupération du nombre total d'articles
...
/**
 * Récupère et affiche le nombre total d'articles depuis l'API
 * Gère l'authentification et la redirection automatique si nécessaire
 * 
 * @return {Promise<number|null>} Le nombre total d'articles ou null en cas d'erreur
 */
async function getTotalArticle() {
	try {
		const res = await fetch(API_URL + '/article/total', {
			method: 'GET',
			credentials: 'include',
			headers: { 
